...
import { getName, is } from '../../../dmn-js-shared/src/util/ModelUtil';
...
export default function DrdRenderer(eventBus, pathMap, styles, textRenderer) {
  ...
  function decisionHasError(decisionId) {
    let ret = false;

    if (this.adInsureOptions && this.adInsureOptions.diagnostics) {
      ret = this.adInsureOptions.diagnostics.getDecisionNumberOfErrors(decisionId) > 0;
    }
  ...
  var handlers = {
    'dmn:Decision': (p, element, attrs) => {

      const hasError = this.decisionHasError(element.businessObject.id);

      if (hasError) {
        attrs = attrs || {};      
        attrs.stroke = "rgb(208, 0, 0)";
      }
    ...
    'dmn:InputData': function(p, element, attrs) {
      ...
      if (element.businessObject.variable && element.businessObject.variable.typeRef.startsWith('adinsure')) {
        attrs = attrs || {};
        attrs.fill = "rgb(213, 222, 232)";
      }
  ...
  this.decisionHasError = decisionHasError;
  this.adInsureOptions = undefined;
