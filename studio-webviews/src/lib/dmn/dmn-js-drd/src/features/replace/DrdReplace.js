...
export default function DrdReplace(drdFactory, replace, selection, modeling) {
  ...
  function replaceElement(element, target, hints) {
    ...
    if (target.expression) {
      ...
      newBusinessObject.decisionLogic.$parent = newBusinessObject;
      newBusinessObject.variable = drdFactory.create('dmn:InformationItem');
      newBusinessObject.variable.$parent = newBusinessObject;
    }

    if (target.context) {
      newBusinessObject.decisionLogic = drdFactory.create('dmn:Context');
      newBusinessObject.decisionLogic.$parent = newBusinessObject;
      newBusinessObject.variable = drdFactory.create('dmn:InformationItem');
      newBusinessObject.variable.$parent = newBusinessObject;
    }
