...
import AdInsureModdle from '../base/moddle/adinsure-moddle.json';
import AdInsureDMNDIModdle from '../base/moddle/adinsure-dmndi-moddle.json';
import AdInsureDIModdle from '../base/moddle/adinsure-di-moddle.json';
...
export default class Manager {
    ...
    constructor(options = {}) {
        ...
        this._viewsChangedInternal = () => {
            this._emit('views.changed', {
                views: this._views,
                activeView: this._activeView,
            });
        };
        this._viewsChanged = debounce(this._viewsChangedInternal, 0);
    ...
    importXML(xml, options, done) {
        ...
        this._moddle.fromXML(xml, 'dmn:Definitions', (err, definitions, context) => {
            ...
            const view = this._activeView || this._getInitialView(this._views);
            ...
            this.open(view, (err, warnings) => {
                const allWarnings = [].concat(parseWarnings, warnings || []);

                this._emit('import.done', { error: err, warnings: allWarnings });

                const decisions = this._getDecisionViews(this._views);

                if (decisions) {
                    let decisionToOpen;
                    if (decisions.length === 1) {
                        decisionToOpen = decisions[0];
                    } else if (options && options.decisionToOpenName){
                        decisionToOpen = decisions.find(d => d.element.name === options.decisionToOpenName);
                    }
                    
                    decisionToOpen && this.open(decisionToOpen);
                }

                done(err, allWarnings, options);
            });
        });
    }
    ...
    destroy() {
        Object.keys(this._viewers).forEach((viewerId) => {
            const viewer = this._viewers[viewerId];
    ...
    _updateViews() {
        ...
        const viewProviders = this._getViewProviders();

        const displayableElements = [definitions, ...(definitions.drgElement || [])];

        // compute list of available views
        this._views = displayableElements.reduce((views, element) => {
            const provider = find(viewProviders, function (provider) {
        ...
        let activeView = this._activeView,
            newActiveView;

        if (activeView) {
            // check the new active view
            newActiveView =
                find(this._views, function (v) {
    ...
    _switchView(newView, done = noop) {
        ...
        const newViewer = newView && this._getViewer(newView),
    ...
    _createViewer(id) {
        const provider = find(this._getViewProviders(), function (provider) {
        ...
        const Viewer = provider.constructor;

        const providerOptions = this._options[id] || {};
        const commonOptions = this._options.common || {};
        ...
        return new Viewer({
            ...
            adInsureOptions: this._options.adInsureOptions,
        });
    }
            ...
            assign(
                {
                    ...
                    adinsure: AdInsureModdle,
                    di: AdInsureDIModdle, // we have to override default di package because of missing `extension` element
                    adinsureDMNDI: AdInsureDMNDIModdle,
                },
...
function checkDMNCompatibilityError(err, xml) {
    ...
    console.error(
        'Cannot open what looks like a DMN ' +
        olderDMNVersion +
        ' diagram. ' +
...
function checkValidationError(err) {
    ...
    err.message =
        ...
        match[1] +
        '> detected; ' +
        'this may indicate an invalid DMN 1.3 diagram file' +
        match[2];
